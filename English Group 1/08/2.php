<?php
    ...
    if ($_POST){
        ...
        else if (str_word_count($name) < 2) // alternatives: strpos, count+explode, substring, ...
...
<body>
    <form action="2.php" method="POST">
        ...
        <?= $errors["name"] ?? "" ?>
        <br>
        <!-- required, at least two words -->

        E-mail: <input type="text" name="email" value="<?= $email ?>">
        <?= $errors["email"] ?? "" ?>
        <br>
        <!-- required, must be the correct format -->
        ...
        <?= $errors["color"] ?? "" ?>
        <br>
        <!-- one of these 4 colors -->

        <input type="checkbox" name="accept" <?= $accept == "on" ? "checked" : "" ?>>I accept everything.
        <?= $errors["accept"] ?? "" ?>
        <br>
        <!-- must be checked -->

        <button type="submit">Submit form</button>
        <!-- if no errors: give a nice message -->
    </form>
</body>
</html>